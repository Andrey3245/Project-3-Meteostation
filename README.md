...
Для сравнения: фотодиод генерирует ток единицы–сотни микроампер. Даже если преобразовать этот ток в напряжение с помощью резистора, полезный сигнал окажется в милливольтах —того же порядка, что и наводки на земле. В результате помехи делают точное измерение невозможным без отдельной чистой аналоговой земли.

### Вывод
Фоторезистору достаточно общей земли с цифровой частью и керамического конденсатора 0.1 мкФ на входе АЦП, который отфильтровывает наводки с проводов.
...
Намотав пару витков провода на антенну передатчика, связь стала устойчивой при закрытой дверце и –25°C. На улице, разумеется, никаких дополнительных витков не нужно — там нет металлических стенок, образующих замкнутый контур. Выдержав длительное тестирование в морозилке, оставалось сделать корпус
  
Схема выдержала длительное тестирование на морозе. Осталась последняя задача — корпус.
